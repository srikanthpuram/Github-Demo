...
class GithubApiClientImpl(private val githubApi: GithubApi) : GithubApiClient {
    ...
    override suspend fun getOwnerCommits(
        owner: String,
        repo: String
    ): Resource<GetGithubCommitsResponseModel> = withContext(Dispatchers.IO) {

        try {
            val response = githubApi.getOwnerCommits("nasa", repo = "mct")
            ...
            Resource.error<GetGithubCommitsResponseModel>("${ex.message}")
        }
    }
}